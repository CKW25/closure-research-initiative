...
status bookkeeping for downstream numerical closure.

At the current manuscript stage, the extracted entry is:
...
the theorem-level closure chain of Chapters~1--21.
Unless further closure theorems are proved, these entries remain explicit
boundary data.

After the global closure audit of Chapters~14--21, the ledger is typed in
three layers.
...
These entries remain outside current theorem-level closure and are tracked as
explicit open boundary data.

\section{Formal hardening ledger}
\label{sec:cond-hardening-ledger}

This section lists every global item that carries non-theorem-level load and
records its strongest current status.
...
The current global closure-status vector is
\[
...
At the present theorem strength, every global non-structural dependency in
...
by polarization from the real quadratic scalar channel.  Thus the old
pairing-level load is represented by \(B\) and is theorem-level.  For mass
...
\begin{corollary}[No hidden global assumptions after hardening]
\label{cor:cond-no-hidden-global-assumptions}
After replacing each occurrence of old assumption-level language by the
corresponding component of \(\mathcal C_{\rm status}\), the manuscript has no
unlisted global quantitative assumption in Chapters~14--22.
\end{corollary}
...
\begin{remark}[Irreducible residue at current theorem strength]
...
a two-reference theorem.  The scalar-pairing and closed-system mass-coordinate bridge theorems are now
proved internally in \cref{thm:scalar-pairing,thm:numbers-mass-calibration}.
