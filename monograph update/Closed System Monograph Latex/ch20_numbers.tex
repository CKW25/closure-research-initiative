...
\item A rigidity envelope for admissible scalar depth observables is proved:
      under intrinsicity and bilinear-origin hypotheses, the only universal
...
can already be constrained by intrinsicity and transport covariance.
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item
      \textbf{Intrinsicity:} \(\Phi(k)\) depends only on the depth-\(k\)
...
behavior at the saved theorem strength.
