...
\orientationitem{Input}{The canonical scalar invariant and the numerical-closure status ledger from \cref{ch:numerical-closure}.}
\orientationitem{Role}{This chapter states the realization boundary for assigning operator targets to matter-sector scalars.}
\orientationitem{Output}{The operator-first spectral realization gap, the conditional promotion checklist, and the observer-map factorization algebra.}
...
the Hilbert branch.
No theorem in the chain above constructs the canonical positive
...
It is not part of the intrinsic theorem package proved above.
\end{remark}
...
If the sector-to-Hilbert embedding and form-typing targets are
...
(\cref{prop:hilbert-real-inner}) together with the required
...
% ============================================================
\subsection{Proof checklist for conditional promotion}
\label{subsec:matter-realization-promotion-checklist}
% ============================================================

This subsection maps the conditional targets of
\cref{subsec:shell-operator-promotion}
to exact proof slots inside the Chapter~22 operator route.

\begin{proposition}[Conditional-target bookkeeping for the Chapter~22 operator route]
\label{prop:matter-conditional-slot-map}
If the conditional targets recorded in
\cref{subsec:shell-operator-promotion}
were discharged, then they would feed this chapter as follows:
\begin{enumerate}[label=\textup{(\roman*)}]
\item
      \cref{thm:conditional-sector-to-hilbert-embedding}
...
\item
      \cref{lem:conditional-embedded-sector-form}
      would supply the form-typing input slot formalized by
      \cref{lem:sector-quadratic-form};
\item
      \cref{thm:conditional-closed-form-closability}
...
\begin{proof}
Immediate from the target status of
\cref{lem:sector-quadratic-form,prop:matter-realization-form-route}
and the promotion criterion recorded in
\cref{subsec:shell-operator-promotion,prop:conditional-implies-ch22-operator-target}.
\end{proof}

\begin{corollary}[Conditional checklist discharge promotes the operator target]
\label{cor:matter-promotion-checklist-to-operator-target}
If the three conditional targets named in
\cref{prop:matter-conditional-slot-map}
are proved from existing theorem-level inputs without new
observer-side assumptions, then that discharge promotes the
...
\begin{proof}
Combine
\cref{prop:matter-conditional-slot-map,prop:matter-realization-form-route,prop:conditional-implies-ch22-operator-target}.
\end{proof}
...
\closingitem{Established}{The canonical sector scalar is intrinsic and operator-realizable through the Hilbert--phase branch, but observer-side mass readings require additional realization data.}
\closingitem{Carried forward}{The completed realization stack is used next to ask the state-side observable-exhaustivity question.}
\end{chapterclosing}
