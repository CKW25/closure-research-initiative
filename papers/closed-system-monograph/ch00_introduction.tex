...
If observers, clocks, laboratories, comparison protocols, and measuring
conventions are all internal to the same world being modeled, then no
background manifold, state-space linearity, causal order, field content,
or observer frame can be accepted as primitive by convention alone.
Such structure is admissible only when it is recovered from intrinsic
relational data, or when the text explicitly marks it as boundary data for
a later realized reading.
...
The purpose of the monograph is to make that requirement into a
mathematical classification problem.
The primitive data are not a manifold, not a Hilbert space, not a field
bundle, and not a causal order.
They are states equipped with binary comparison predicates.
The central question is then structural:
which objects can be reconstructed from those predicates, which apparent
distinctions fail to descend through the internal symmetries, and which
additional data must remain explicitly marked at a realization interface?
...
Geometric, field-theoretic, scalar, matter-sector, and state-side
reconstruction statements then enter downstream from those constraints.
The direction of explanation is therefore reversed from the usual
background-first presentation: the manuscript does not begin with a
space on which fields live, but with comparison data whose closure
conditions determine which geometric and field-theoretic readings are
admissible.
...
for closed relational systems satisfying the stated hypotheses, the
admissible global architecture is determined by the comparison and
transport stack rather than by an independently supplied background.
