...
\orientationitem{Input}{The scalar-depth package and quartic ratio geometry developed in Chapters~\ref{ch:matter}--\ref{ch:matter-numbers}.}
\orientationitem{Role}{This chapter separates intrinsic numerical closure from realization-level normalization and same-channel correction bookkeeping.}
\orientationitem{Output}{The unified scalar-channel closure chain, same-channel correction structure, and the final numerical-closure statement.}
...
The separation is important: the intrinsic scalar channel fixes a
normalization orbit and same-channel correction structure, while a
particular physical calibration remains a realization-level choice.
...
Second comes a compatibility bridge that preserves shell labels from the
one-reference notation
...
The unconditional intrinsic statements in this chapter use two established
upstream inputs:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item the Chapter~20 quartic-backbone and rigidity-envelope package,
      read with its stated distinction between structural consequences
...
The quartic growth envelope and quartic leading law are the Chapter~20
consequences used here; they do not by
...
\begin{corollary}[Intrinsic prediction content at stated strength]
...
\begin{theorem}[Complete intrinsic spectral prediction at the stated split]
...
% ============================================================
\subsection{Conditional promotion criterion for the operator target}
\label{subsec:shell-operator-promotion}
% ============================================================
...
The following labeled items record the minimal conditional targets whose
discharge would suffice for that promotion: canonical sector-to-Hilbert
embedding, induced sector form construction, and closed-form closability.

\begin{theorem}[Conditional target: canonical sector-to-Hilbert embedding]
\label{thm:conditional-sector-to-hilbert-embedding}
...
\begin{lemma}[Conditional target: embedded sector form realizes intrinsic scalar values]
\label{lem:conditional-embedded-sector-form}
Assuming \cref{thm:conditional-sector-to-hilbert-embedding}, promotion to
intrinsic status would also be secured by proving the following target
statement from
...
\begin{theorem}[Conditional target: closed-form closability from intrinsic control]
\label{thm:conditional-closed-form-closability}
Assuming \cref{lem:conditional-embedded-sector-form}, promotion to intrinsic
status would further be secured by proving this target statement:
...
\begin{proposition}[Conditional-target implication for the Chapter~22 operator target]
\label{prop:conditional-implies-ch22-operator-target}
If
\cref{thm:conditional-sector-to-hilbert-embedding,lem:conditional-embedded-sector-form,thm:conditional-closed-form-closability}
were discharged from existing theorem-level inputs alone, then
...
\begin{proof}
A proof of the first two conditional targets would provide the
sector-form data required by \cref{lem:sector-quadratic-form}.
A proof of the third would provide the closed/closable-form
hypothesis used by \cref{prop:matter-realization-form-route}.
Together, those discharges would therefore yield the canonical
...
\begin{corollary}[Promotion criterion at conditional-target strength]
\label{cor:conditional-promotion-criterion}
A full promotion from realization/conditional to intrinsic/forced for the
Chapter~22 operator statement is justified exactly when the conditional
targets
\cref{thm:conditional-sector-to-hilbert-embedding,lem:conditional-embedded-sector-form,thm:conditional-closed-form-closability}
are proved without adding new observer-side assumptions.
\end{corollary}

\begin{proof}
Immediate from
\cref{prop:conditional-implies-ch22-operator-target,sec:cond-open}.
\end{proof}

% ============================================================
\section{Compatibility bridge for one-reference shell labels}
\label{sec:shell-one-reference-compat}
% ============================================================

For consistency with the existing ledger and cross-chapter references, we
record a compact compatibility layer in one-reference shell notation.
These labels are optional interface markers and do not add observer-side
inputs to the intrinsic core established above.
They function as interface-level translators between the closure
language and one-reference shell bookkeeping.
...
\begin{definition}[Shell completeness property (one-reference label)]
...
\begin{definition}[One-reference defect-evaluation datum]
...
\begin{definition}[One-reference anchor datum]
...
The intrinsic numerical sector is closed at theorem level:
the scalar law is fixed up to one normalization orbit, and any
...
core of the main manuscript at the stated theorem strength: quartic leading
...
\closingitem{Established}{The intrinsic one-channel numerical sector is closed up to one normalization orbit, with same-channel corrections kept as explicit compatibility models.}
\closingitem{Carried forward}{The scalar's branch-level operator and observer-map realization are treated separately.}
\end{chapterclosing}
