...
\orientationitem{Input}{Fixed quotient semantics together with the static transport obstruction identified by the rigidity bridge.}
\orientationitem{Role}{This chapter studies internal observer reductions without adding an external time or external coarse graining.}
\orientationitem{Output}{A conditional observer-level irreversibility criterion, an extended pseudo-ultrametric, and the two-locus residue carried forward to spacetime interleaving.}
...
\closingitem{Established}{Irreversibility is localized to observer-induced descent after fixed reversible quotient dynamics, not to the intrinsic quotient map itself.}
\closingitem{Carried forward}{The observer-side filtration is stabilized next inside a joint refinement--time arena.}
\end{chapterclosing}
